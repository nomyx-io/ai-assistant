...
class AssistantAPI extends events_1.EventEmitter {
    setState(newState) {
    ...
    constructor(serverUrl = 'https://api.openai.com/v1/') {
        super();
        this.prompt = `You are a helpful. highly-skilled assistant enabled with a number of powerful tools.
# Application State
...
`; // state
        ...
        this.actionHandlers = {
            "runs-create": {
                "action": (data, state) => this.callAPI('runs', 'retrieve', {
            ...
            "runs-list_run_steps": {
                "action": async (data, state) => {
                    ...
                    const run_steps = results.reduce((acc, result) => {
            ...
            "execute-tools": {
                action: async (data, state) => {
                    ...
                    for (const tool_call of tool_calls) {
                        ...
                        if (func) {
                            const result = await func(JSON.parse(tool_call[tool_call.type].arguments), state);
            ...
            "cleanup-old": {
                action: async (data, state) => {
                    ...
                    assistants = assistants.find((assistant) => assistant.name === this.name);
        ...
        this.tools = {
            state: ({ name, value }, state) => {
                ...
                if (!value) {
                    ...
                    state[name] = value;
                    out = `${name} => ${JSON.stringify(state[name])}`;
                    this.setState(state);
                }
                return out || 'undefined';
            },
            states: function ({ values }, state) {
                const results = [];
            ...
            set_tasks: function ({ tasks }, state) {
                state.tasks = tasks;
    ...
    async callAPI(type, api, params = {}, callDelay = 0, retries = 3, retryDelay = 1000) {
        ...
        const url = new url_1.URL(path, this.serverUrl);
        try {
            if (this.debug) {
            ...
            const response = await (0, node_fetch_1.default)(url, reqData);
            if (response.ok) {
    ...
    waitThenEmit(event, data, delay) {
        setTimeout(() => this.emit(event, data), delay);
    }
    setStatusThenEmit(event, data, status) {
        this.setState({
            run: data,
            status: status
        });
    ...
    setupActionHandler(handlerName, action, nextState) {
        this.actionHandlers[handlerName] = { action, nextState };
        this.on(handlerName, async (data) => {
    ...
    async runActionHandler(handlerName, data) {

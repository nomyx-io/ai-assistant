...
class TerminalSessionManager {
    constructor() {
        ...
        this.actionHandlers = {
            ...
            // ********** action handlers **********
            "run-completed": {
                action: ({ run, percent_complete: pc2 }, { requirements, tasks, threads, percent_complete }, assistantAPI) => __awaiter(this, void 0, void 0, function* () {
                    const messages = yield assistantAPI.callAPI('messages', 'list', { thread_id: run.thread_id });
            ...
            "session-init": {
                action: ({ run: run_param }, { assistant, thread, run }, assistantAPI) => __awaiter(this, void 0, void 0, function* () {
                    const config = loadConfig();
                    assistantAPI.apiKey = config.apiKey || process.env.OPENAI_API_KEY;
                    // thread
                    if (config.thread_id) {
                        thread = yield assistantAPI.callAPI('threads', 'retrieve', { thread_id: config.thread_id });
                    }
                    else {
                        thread = yield assistantAPI.callAPI('threads', 'create', { body: {} });
                    }
                    // run
                    if (config.run_id) {
                        const run = yield assistantAPI.callAPI('runs', 'retrieve', { thread_id: thread.id, run_id: config.run_id });
                        ...
                        if (run.status === 'active' || run.status === 'requires_action') {
                            ...
                            yield assistantAPI.callSync('runs-cancel', { run });
                        }
                    }
                    ...
                    yield assistantAPI.callSync('session-start', sessionInfo);
                }),
                nextState: null
            },
            "session-start": {
                action: ({ assistant, thread, run }, state) => __awaiter(this, void 0, void 0, function* () {
            ...
            "send-message": {
                action: ({ message, thread }, { assistant, run, requirements, percent_complete = 0, status = 'in progress', tasks = [], current_task = '', chat = '' }, assistantAPI) => __awaiter(this, void 0, void 0, function* () {
                    const inputFrame = {
                    ...
                    if (!thread) {
                        thread = yield assistantAPI.callAPI('threads', 'create', {
                            body: {}
                        });
                    }
                    // cancel any active runs
                    let runs = yield assistantAPI.callAPI('runs', 'list', { thread_id: thread.id });
                    if (runs.data.length > 0) {
                        yield Promise.all(runs.data.map((run) => __awaiter(this, void 0, void 0, function* () {
                            if (run.status === 'active' || run.status === 'requires_action') {
                                yield assistantAPI.callAPI('runs', 'cancel', { thread_id: run.thread_id, run_id: run.id });
                            }
                        })));
                    }
                    // create a new message and run
                    yield assistantAPI.callAPI('messages', 'create', {
                        thread_id: thread.id,
                        body: { role: 'user', content: JSON.stringify(inputFrame) }
                    });
                    run = yield assistantAPI.callAPI('runs', 'create', {
                        thread_id: thread.id,
                    ...
                    yield assistantAPI.callSync('run-queued', { run });
                }),
                nextState: null
            },
            "run-cancel": {
                action: ({ run }, state, assistantAPI) => __awaiter(this, void 0, void 0, function* () {
                    if (run.status === 'active' || run.status === 'requires_action') {
                        yield assistantAPI.callAPI('runs', 'cancel', { thread_id: run.thread_id, run_id: run.id });
                    }
    ...
    initAssistant(terminalSession) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (config.assistant_id) {
                try {
                    assistant = yield terminalSession.callAPI('assistants', 'retrieve', { assistant_id: config.assistant_id });
                }
                catch (e) {
                    console.log('Assistant not found. Creating a new assistant.');
                    assistant = yield terminalSession.callAPI('assistants', 'create', {
                        body: {
...
class TerminalSession extends assistant_1.default {
    constructor(manager) {
        ...
        Object.entries(manager.actionHandlers).forEach(([handlerName, handler]) => {
            ...
            try {
                this[handler.handleType || 'on'](handlerName, (data) => __awaiter(this, void 0, void 0, function* () {
                    if (this.beforeAction)
                        yield this.beforeAction(handlerName, data, this.state, this);
                    yield handler.aaction(data, this.state, this);
                    if (this.afterAction)
                        yield this.afterAction(handlerName, data, this.state, this);
                    if (handler.nextState) {
                        this.emit(handler.nextState, this.state);
                    }
                }), this);
            }
            catch (error) {
...
//# sourceMappingURL=multisession_backup.js.map
...
export class Tool extends EventEmitter {
  ...
  constructor(registry: any, name: string, version: string, description: string, source: string, tags: string[], schema: any) {
    super();
  ...
  executor(): (params: any, api: Assistant) => Promise<any> {
    return async (params: any, api: Assistant) => {
      const ff = new Function('params', 'api', `return (async () => { return ${this.source} })();`);
      return await ff(params, api);
    };
  }
...
class ToolRegistry extends EventEmitter {
  ...
  private saveRegistry(): void {
    try {
      const registryDataWithoutCircular = removeCircularReferences(this.registryData);
      const registryDataWithoutFunctions = registryDataWithoutCircular
...
You output only RAW JAVASCRIPT, WITHOUT ANY COMMENTARY, EXPLANATION or FORMATTING`
  ...
  async getToolVersion(name: string, version: string): Promise<Tool | null> {
    ...
    return new Tool(this, tool.name, version, tool.description, source, tool.tags, tool.schema);
  }
...
`;
  ...
  async addTool(name: string, source: string, schema: any, tags: string[]): Promise<boolean> {
    try {
      ...
      const version = '1.0.0';
      const newTool = new Tool(this, name, version, schema.description, source, tags, schema);
